// ============================================================================
// BokPiloten – Worker v20 "32p Spread-Perfect"
...
// Try multiple sources for TTFs (Baloo2 / Nunito). Fall back to standard fonts.
...
async function buildPdf(
  ...
  try {
    ...
    const fitT = shrinkToFit(title, baloo, maxTitle, minTitle, titleWidth, 2);
    const titleLH = fitT.size * 1.08;
    const fitS = subtitle ? shrinkToFit(subtitle, nunitoSemi, Math.max(14, fitT.size * 0.46), 12, titleWidth, 2) : null;
    const subLH = fitS ? fitS.size * 1.12 : 0;
    const blockH = fitT.lines.length * titleLH + (fitS ? (mmToPt(4) + fitS.lines.length * subLH) : 0);
    let cy = contentY + trimHpt - mmToPt(GRID.outer_mm + 6) - blockH;

    // Gradient toppbar för läsbarhet
    for (let i = 0; i < 8; i++) {
      const t = (8 - i) / 8;
      pdfDoc.getPage(0).drawRectangle({ x: contentX, y: contentY + trimHpt - mmToPt(20) + (mmToPt(20)/8)*i, width: trimWpt, height: mmToPt(20)/8, color: rgb(0,0,0), opacity: 0.22 * t });
    }
  ...
  /* ------- [1] TITELBLAD ------- */
  {
    ...
    drawIconStar(page, cx, contentY + mmToPt(18), mmToPt(14), rgb(0.1,0.2,0.4), 0.25);
    if (mode === "preview" && watermark_text) drawWatermark(page, watermark_text);
  }
  ...
  for (let i = 0; i < 14; i++) {
    ...
    drawWrappedCenterColor(rightTextPage, text, cx, cy, trimWpt*0.76, trimHpt*0.46, nunito, baseTextSize, baseLeading, 12, rgb(0.08,0.08,0.1), "center");
    // Dekor + sidnummer
    drawIconHeart(rightTextPage, cx, contentY + mmToPt(20), mmToPt(18), rgb(0.2,0.25,0.5), 0.25);
    const pageNum = 2 + i*2 + 1; // höger inlaga (tryck-nr internt)
  ...
  /* ------- [30] SLUT-sida ------- */
  {
    ...
    drawIconStar(page, cx, contentY + mmToPt(20), mmToPt(18), rgb(0.2,0.25,0.5), 0.25);
    if (mode === "preview" && watermark_text) drawWatermark(page, watermark_text);
  }
...
Returnera enbart json.`.trim();
      ...
      // Generate interior images (optionally batched via pages_subset)
